    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('Email not confirmed')) {
          toast.error(
            'Email belum dikonfirmasi. Silakan check email Anda atau hubungi admin untuk mengaktifkan akun.',
            { duration: 6000 }
          )
        } else {
        ...
        <div className="mt-6 space-y-3">
          <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
            <p className="text-sm text-blue-800">
              💡 <strong>Tip:</strong> Jika Google login tidak bekerja, pastikan sudah enable Google OAuth di Supabase Dashboard.
            </p>
          </div>
          
          <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
            <p className="text-sm text-yellow-800 mb-2">
              ⚠️ <strong>Email Not Confirmed?</strong>
            </p>
            <p className="text-xs text-yellow-700">
              Jika muncul error "Email not confirmed", disable email confirmation di Supabase:
              <br />
              <strong>Authentication → Providers → Email → Uncheck "Confirm email"</strong>
              <br />
              Atau baca file <code className="bg-yellow-100 px-1 rounded">DISABLE_EMAIL_CONFIRMATION.md</code>
            </p>
          </div>
        </div>
      </div>
